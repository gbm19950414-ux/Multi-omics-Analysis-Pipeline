...
## ============================================================
## 02_rna_upstream_pathway_scores.R
##
## 目的：
##   基于基因层面 DESeq2 log2FC（KO vs WT），
##   使用上游通路基因集 YAML (rna_upstream_pathway_sets.yaml)，
##   计算每个上游通路在每个 batch 的 HO vs WT 效应 Z 分数：
...
##
##   2) 上游通路基因集：
##        scripts/multi/rna_upstream_pathway_sets.yaml
##
##   3) 可选配置文件（命令行第 1 个参数）：
##        scripts/rna/00_rna_upstream_pathway_config.yaml
##
##      支持字段：
##        deseq2_outdir: "results/rna/deseq2"
##        upstream_gene_set_yaml: "scripts/multi/rna_upstream_pathway_sets.yaml"
##        lfc_table: "interaction_summary_with_perBatchLFC.tsv"
##        outdir: "results/rna/upstream_pathways"
##
## 输出：
##   - results/rna/upstream_pathways/rna_upstream_pathway_scores_per_batch.tsv
##     列：
##       pathway, axis, batch,
##       n_genes, mean_LFC, sd_LFC, se_LFC, z_pathway
##
##   z_pathway 的含义：
##     在该 pathway 内所有基因的 HO vs WT 有效 log2FC（按 gene_effect_sign 校正）
##     的均值，用 pathway 内基因间变异估计不确定性（SE），
##     z_pathway = mean_LFC / se_LFC。
##
## ============================================================
...
upstream_yaml_default <- "scripts/multi/rna_upstream_pathway_sets.yaml"

## ⚠️ 这里的默认文件名需要和你实际的 LFC 表一致：
...
upstream_yaml <- cfg$upstream_gene_set_yaml%||% upstream_yaml_default
lfc_table     <- cfg$lfc_table             %||% lfc_table_default

lfc_path <- file.path(deseq2_outdir, lfc_table)

cat("============================================================\n")
cat("[INFO] 02_rna_upstream_pathway_scores.R\n")
cat("  LFC table path : ", lfc_path, "\n", sep = "")
cat("  pathway YAML   : ", upstream_yaml, "\n", sep = "")
cat("  outdir         : ", outdir, "\n", sep = "")
cat("============================================================\n\n")
...
if (!file.exists(upstream_yaml)) {
  stop("[ERROR] 找不到上游通路基因集 YAML: ", upstream_yaml)
}

cat("\n[STEP2] 读取上游通路 YAML: ", upstream_yaml, " ...\n", sep = "")
pw_cfg <- yaml::read_yaml(upstream_yaml)

if (is.null(pw_cfg$pathways)) {
  stop("[ERROR] 上游通路 YAML 中未找到 'pathways' 顶层字段。")
}
...
yaml_dir  <- dirname(upstream_yaml)
yaml_base <- tools::file_path_sans_ext(basename(upstream_yaml))
registry_path <- file.path(yaml_dir, paste0(yaml_base, "_gene_registry.tsv"))

if (!file.exists(registry_path)) {
  stop("[ERROR] 找不到基因注册表文件: ", registry_path,
       "\n请先运行 09a.1_update_gene_symbol_to_ensembl.R 生成该文件。")
}
...
cat("\n[STEP4] 计算每个 pathway × batch 的 mean_LFC / Z 分数 ...\n")

pathway_scores <- pw_lfc %>%
  group_by(pathway, axis, batch) %>%
  summarise(
...
out_path <- file.path(outdir, "rna_upstream_pathway_scores_per_batch.tsv")
readr::write_tsv(pathway_scores, out_path)

cat("\n[OK] 写出上游通路 Z 分数表: ", out_path, "\n", sep = "")
cat("============================================================\n")
cat("[DONE] 02_rna_upstream_pathway_scores.R 完成。\n")
cat("  说明：\n")
cat("    - mean_LFC > 0 且 z_pathway > 0：通路在 HO 中整体激活；\n")
cat("    - mean_LFC < 0 且 z_pathway < 0：通路在 HO 中整体受抑；\n")
cat("    - 绝对值越大，效应越强，可用于和机制轴 Z 做方向/强度匹配。\n")
cat("============================================================\n")